...
const updatePosition = () => {
  ...
  let y = props.anchorY - (cardHeight.value / 2)
...
watch(() => [props.anchorX, props.anchorY], () => {
  updatePosition()
})
...
<template>
  <Teleport to="body">
    <Transition name="fade">
      <div
        v-if="isOpen && slotData && definition"
        class="fixed inset-0 z-[150]"
        @click="handleBackdropClick"
      >
        <div
          ref="cardRef"
          class="fixed w-[280px] select-none"
          :style="{
            left: `${position.x}px`,
            top: `${position.y}px`
          }"
        ...
        >
          <div class="relative rounded-xl border-2 border-solid border-slate-500/50 shadow-[0_8px_32px_rgba(0,0,0,0.6),inset_0_1px_0_rgba(255,255,255,0.1)] overflow-hidden">
            ...
            <button
              class="absolute top-2 right-2 z-10 w-6 h-6 rounded-full bg-slate-700/80 border border-solid border-slate-500/50 flex items-center justify-center text-slate-400 hover:text-white hover:bg-slate-600/80 transition-all duration-150"
              @click="handleClose"
            >
            ...
            <div class="relative p-4 flex gap-4">
              ...
              <div class="flex flex-col justify-center min-w-0">
                <h3 class="text-white font-bold text-base truncate pr-6">
                  {{ displayLabel }}
                </h3>
            ...
            <div v-if="definition.description" class="relative px-4 pb-4">
              ...
              <div v-if="filteredMetadata" class="mt-3 mx-auto w-3/4 h-[1px] bg-gradient-to-r from-transparent via-slate-400 to-transparent" />
            </div>
